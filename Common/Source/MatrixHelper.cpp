...
static void setRotateM(GLMatrix& result, int rmOffset,
...
void rotateM(GLMatrix& result, GLfloat angle, GLfloat x, GLfloat y, GLfloat z) {
    ...
    // 创建临时对象
    GLMatrix *temp = (GLMatrix *)malloc(sizeof(GLMatrix));
    memset(temp, 0, sizeof(GLMatrix));
    GLMatrix *ret = (GLMatrix *)malloc(sizeof(GLMatrix));
    memset(ret, 0, sizeof(GLMatrix));
    // 设置旋转矩阵
...
int orthoM(GLMatrix &result, int mOffset,
    ...
    float r_depth = 1.0f / (_far - _near);
    float x = 2.0f * (r_width);
    float y = 2.0f * (r_height);
    float z = -2.0f * (r_depth);
    float tx = -(right + left) * r_width;
    float ty = -(top + bottom) * r_height;
    float tz = -(_far + _near) * r_depth;
...
int frustumM(GLMatrix& result, int offset, float left, float right,
    ...
    float r_width = 1.0f / (right - left);
    float r_height = 1.0f / (top - bottom);
    float r_depth = 1.0f / (_near - _far);
    float x = 2.0f * (_near * r_width);
    float y = 2.0f * (_near * r_height);
    float A = (right + left) * r_width;
    float B = (top + bottom) * r_height;
    float C = (_far + _near) * r_depth;
    float D = 2.0f * (_far * _near * r_depth);
    result.m[offset + 0] = x;
    result.m[offset + 5] = y;
    result.m[offset + 8] = A;
    result.m[offset + 9] = B;
...
int perspectiveM(GLMatrix&result, int offset,
